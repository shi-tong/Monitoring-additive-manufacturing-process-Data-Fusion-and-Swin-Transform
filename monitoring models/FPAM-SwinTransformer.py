...
import torch.nn as nn
import torch.optim as optim
from torch.optim import lr_scheduler  # 添加了lr_scheduler的导入
import torch.nn.functional as F
import torch.nn as nn
class Mlp(nn.Module):
...
class WindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., use_sparse_attention=True):
        ...
        if self.use_sparse_attention:
            self.relative_position_index = self.relative_position_index.to_sparse()
    ...
    def forward(self, x, mask=None):
        ...
        if self.use_sparse_attention:
            attn_sparse = torch.sparse_coo_tensor(self.relative_position_index.view(1, -1),
                                                  attn.view(-1), (B_ * self.num_heads, N * N))
            attn_sparse = attn_sparse.coalesce()
            attn = attn_sparse.to_dense().view(B_, self.num_heads, N, N)
...
class FocusedLinearAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.dwc = nn.Sequential(
            nn.Conv2d(in_channels=head_dim, out_channels=head_dim, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=head_dim, out_channels=head_dim, kernel_size=3, groups=head_dim, padding=1),
            nn.Conv2d(in_channels=head_dim, out_channels=head_dim, kernel_size=1)
        )
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # FFN with depthwise separable convolution and GELU activation
...
class ExpandedConv(nn.Module):
    def __init__(self, in_channels, out_channels, expansion_factor):
        ...
        self.depthwise = nn.Conv2d(in_channels, mid_channels, kernel_size=3, stride=1, padding=1, groups=in_channels)
        self.pointwise = nn.Conv2d(mid_channels, out_channels, kernel_size=1)
